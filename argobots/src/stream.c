...
int ABT_xstream_check(ABT_pool pool, int *run_cnt, ABT_sched sched)
{
    int abt_errno = ABT_SUCCESS;
    ABTI_pool *p_pool = ABTI_pool_get_ptr(pool);
    uint64_t id = p_pool->id;
    ABT_unit unit;
    ...
    {
        abt_errno = ABT_my_xstream_run_unit(unit, pool, sched);
        run_cnt++;
    }
}

int ABT_my_pools_of_sched_0(ABT_sched sched, ABT_pool *pools)
{
    ABTI_sched *p_sched = ABTI_sched_get_ptr(sched);
    printf("sched %lu :", p_sched->id);
    for(int i=0; i<41;i++)
    {
        printf("%lu ", ABTI_pool_get_ptr(pools[i])->id);
    }
    printf("\n");
}

int ABT_my_pools_of_sched(ABT_sched sched, ABT_pool *pools)
{
    ABTI_sched *p_sched = ABTI_sched_get_ptr(sched);
    printf("sched %lu :", p_sched->id);
//#if 0
...
int ABT_my_migrate(ABT_pool *pools, ABT_sched sched, int *run_cnt)
{
    int target;
    size_t size;

    ABT_pool pool = pools[1];
...
//    printf("sched_id=%lu", p_sched->id);
//    for(int i =0;i<41;i++)
//        printf(" %lu", ABTI_pool_get_ptr(pools[i])->id);
//    printf("\n");

    uint64_t sched_id = p_sched->id;
    if(p_pool->id > 0)
    {
        if(sched_id < 22)
            target = 22 - sched_id;
...
//#if 0
        while(size > 0)
        {
            ...
            if(unit != ABT_UNIT_NULL){
                ...
                //printf("sched=%lu, pop from %lu, push to %lu\n", p_sched->id, ABTI_pool_get_ptr(pools[target])->id, ABTI_pool_get_ptr(pools[1])->id);
...
int ABT_my_xstream_run_unit(ABT_unit unit, ABT_pool pool, ABT_sched sched)
{
    int abt_errno;
    ABTI_xstream *p_xstream = ABTI_local_get_xstream();
    ABTI_pool *p_pool = ABTI_pool_get_ptr(pool);
    ...
    abt_errno = ABTI_my_xstream_run_unit(p_xstream, unit, p_pool, p_sched);
    ABTI_CHECK_ERROR(abt_errno);
...
int ABTI_my_xstream_run_unit(ABTI_xstream *p_xstream, ABT_unit unit,
                             ABTI_pool *p_pool, ABTI_sched *p_sched)
{
    int abt_errno = ABT_SUCCESS;
    ...
    if(type == ABT_UNIT_TYPE_THREAD){
        ...
        //printf("es=%lu, pool=%lu, thread=%lu, main_sched=%lu, sched=%lu\n", p_xstream->rank, p_pool->id, p_thread->id, p_xstream->p_main_sched->id, p_sched->id);
                          ...
                          ABTI_pool *p_pool)
{
    ...
    if (type == ABT_UNIT_TYPE_THREAD) {
        ...
        ABTI_thread *p_thread = ABTI_thread_get_ptr(thread);
        /* Switch the context */
        //printf("xstream=%lu, pool=%lu, thread=%lu, main_sched=%lu\n", p_xstream->rank, p_pool->id, p_thread->id, p_xstream->p_main_sched->id);
        abt_errno = ABTI_xstream_schedule_thread(p_xstream, p_thread);
        ABTI_CHECK_ERROR(abt_errno);
